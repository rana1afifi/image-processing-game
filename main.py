...
def getPeaks(processedImage, background, diffThreshHoldConst, originalImage, cutOffFreq, isVideoFrame=True, index = -1 , name="Camera"):
    pickedPeaks = None
    humanCenter = None
    ...
    if isVideoFrame:
        ...
        erosionVid = cv2.erode(diffThreshold, dilationFilter, iterations=5)
        dilationVid = cv2.dilate(erosionVid, dilationFilter, iterations=5)
        if debugMode:
...
def getFixedImagesPeaks(images, bgFilename, blurFactor, diffThreshHoldConst, cutOffFreq):
    ...
    for index, imgFilename in enumerate(images):
        ...
        peaks[index], humanCenters[index] = getPeaks(processed_img, background, diffThreshHoldConst, image, cutOffFreq, False, index)
...
def drawFunction(originalVid,peaks,humanCenter):
    ...
    # pts = np.array([[50, 50], [25, 70], [150, 100], [170, 80]], np.int32)
    # pts = pts.reshape((-1, 1, 2))
    # cv2.polylines(displayedScreen, [pts], True, (0, 255, 255))
...
def main():
    ...
    fixedPeaks, fixedCenters = getFixedImagesPeaks(fixedPoses, 'bg.jpg', blurFactor, diffThreshHoldConst, cutOffFreq)
...
#       ret = True
        ...
        if gameType == 0:  #Display All Mode
            ...
            if timeDiff > displayTime:
                ...
                t0 = time.time()
                print("game type is ", gameType)
                cv2.destroyWindow("With Circle "+str(False))
            ...
            if matching>=3:
                showStage([], score=score, result=1) #TODO write won
            else:
                showStage([], score=score, result=2) #TODO write lose
